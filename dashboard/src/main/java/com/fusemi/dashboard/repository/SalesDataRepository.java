...
public interface SalesDataRepository extends JpaRepository<SalesData, Long> {
    ...
    @Query("SELECT COALESCE(SUM(s.amount), 0) FROM SalesData s WHERE s.saleDate BETWEEN :start AND :end")
    BigDecimal sumAmountByDateRange(@Param("start") LocalDate start, @Param("end") LocalDate end);

    @Query("SELECT COUNT(DISTINCT s.customerName) FROM SalesData s WHERE s.saleDate BETWEEN :start AND :end")
